...
Background:
Given a user with email "test@example.com" and username "tesuser" is logged into the system
...
When the user "tesuser" marks the listing with ISBN "999-9-9-9999999-0" as sold
...
When the user "tesuser" attempts to mark the listing with ISBN "999-9-9-9999999-0" as sold again
...
When the user "tesuser" attempts to mark the listing with ISBN "092-5-2-1111111-9" as sold
...
When the user "tesuser" attempts to mark the listing with ISBN "123-4-5-6789101-1" as sold
...
When the user "tesuser" marks the following listings as sold:
  | ISBN              |
  | 999-9-9-9999999-0 |
  | 888-8-8-8888888-0 |
Then the listings should be updated with:
  | bookName | ISBN              | listingStatus |
  | mybook   | 999-9-9-9999999-0 | Sold         |
  | mybook2  | 888-8-8-8888888-0 | Sold         |
...
When the user "tesuser" attempts to mark the listing with ISBN "999-9-9-9999999-0" as sold
...
When the user "tesuser" attempts to change the status back to "Available"
