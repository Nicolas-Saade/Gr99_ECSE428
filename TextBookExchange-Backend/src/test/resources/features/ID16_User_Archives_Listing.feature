...
Background:
Given a user with email "test2@example.com" and username "test2user" is logged into the system2
And the following listings exist2:
  | bookName        | ISBN                 | bookCondition | listingStatus | price   | datePosted  | course   |
  | mybook          | 999-9-9-9999999-1    | New           | Available     | 18.09   | 2025-02-01  | ECSE 428 |
  | mybook2         | 888-8-8-8888888-1    | Used          | Available     | 209.66  | 2025-01-23  | ECSE 429 |


Scenario: User marks their listing as sold (Normal Flow)

When the user "test2user" marks the listing with ISBN "999-9-9-9999999-1" as sold
Then the listing should be updated with:
  | bookName | ISBN              | listingStatus |
  | mybook   | 999-9-9-9999999-1 | Unavailable         |


Scenario: User tries to mark a listing as sold that has already been marked as sold (Error Flow)

Given the listing with ISBN "999-9-9-9999999-1" is already marked as "Sold"
When the user "test2user" attempts to mark the listing with ISBN "999-9-9-9999999-1" as sold again
...
When the user "test2user" attempts to mark the listing with ISBN "092-5-2-1111111-9" as sold
...
When the user "test2user" attempts to mark the listing with ISBN "123-4-5-6789101-1" as sold
...
When the user "test2user" marks the following listings as sold:
  | ISBN              |
  | 999-9-9-9999999-1 |
  | 888-8-8-8888888-1 |
Then the listings should be updated with:
  | bookName | ISBN              | listingStatus |
  | mybook   | 999-9-9-9999999-1 | Unavailable         |
  | mybook2  | 888-8-8-8888888-1 | Unavailable         |
...
When the user "test2user" attempts to mark the listing with ISBN "999-9-9-9999999-1" as sold
...
When the administrator marks the listing with ISBN "999-9-9-9999999-1" as sold
Then the listing should be updated with:
  | bookName | ISBN              | listingStatus |
  | mybook   | 999-9-9-9999999-1 | Unavailable         |
...
Given the listing with ISBN "999-9-9-9999999-1" is currently marked as "Sold"
When the user "test2user" attempts to change the status back to "Available" for ISBN "999-9-9-9999999-1"
